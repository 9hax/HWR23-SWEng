...
class User(db.Model):
    ...
    userDocument = db.relationship('Document', backref='existing documents', foreign_keys=[documentID])
    def __repr__(self):
...
class Document(db.Model):
    ...
    fromOfficeID = db.Column(db.Integer, db.ForeignKey('user.id'))
    hidden = db.Column(db.Boolean, unique=False, default= False)

    fromOffice = db.relationship('Office', backref='tickets_created_by', foreign_keys=[fromOfficeID])
    def __repr__(self):
        return '<Ticket %r>' % self.title

class DocumentReply(db.Model):
    id = db.Column(db.Integer, primary_key = True)
    text = db.Column(db.Text, unique=False, nullable=False)
    ...
    send_to_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    ...
    send_to = db.relationship('User', backref='ticket_reply_by', foreign_keys=[created_by_id])
